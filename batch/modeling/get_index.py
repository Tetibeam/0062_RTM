...
def _get_fred_prices_test(ticker: str, name:str, start_date:pd.Timestamp, end_date: pd.Timestamp) -> pd.DataFrame:
    ...
    try:
        ...
        fred = Fred(api_key="d8028eba4732e356349912d4e0f07dc3")
        df = fred.get_series(ticker,observation_start="2015-01-01")
        df.name = name
        df.index.name ="Date"
...
if __name__ == "__main__":
    ...
    df = _get_yfinance_prices_test(ticker="DX=F", name="DX=F", start_date=start, end_date=end)
    #df = _get_fred_prices_test(ticker="CP", name="CP", start_date=start, end_date=end)
