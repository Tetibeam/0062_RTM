...
# メインプロセス
########################################################
def get_mgi_index_model_beta(df_index):
    # --- データの取得：マスターデータからLiq_eff_modelに必要なデータを取り出す ---
    df = df_index[mgi_index]
...
def _make_label(target, df_index):
    ...
    for LAG in [8]:
        ...
        # ダイナミック閾値
        df['dynamic_q_low'] = df['future_MGI'].rolling(window=window, min_periods=52).quantile(quantile_low)
        df['dynamic_q_high'] = df['future_MGI'].rolling(window=window, min_periods=52).quantile(quantile_high)
...
def _analysis_label(df):
    ...
    for start, end in terms:
        ...
        stats = df_term['MGI_label'].value_counts().to_frame(name='Count')
        stats['Percentage (%)'] = (df_term['MGI_label'].value_counts(normalize=True) * 100).round(2)
        print(stats)
        ...
        #print(df[df["MGI_label"]==1.0].index)

    """# 継続日数の算出
    df['change'] = df['MGI_label'] != df['MGI_label'].shift()
    df['regime_id'] = df['change'].cumsum()

    # 各期間の長さをカウント
    duration_stats = df.groupby(['regime_id', 'MGI_label']).size().reset_index(name='duration')
